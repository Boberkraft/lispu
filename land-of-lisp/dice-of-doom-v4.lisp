...
;;                  ATACKING   2    3    4    5   DICES
(defparameter *dice-odds* #(#(0.84 0.97 1.0  1.0 ); 1
                            #(0.44 0.78 0.94 0.99); 2
                            #(0.15 0.46 0.74 0.91); 3
                            #(0.04 0.19 0.46 0.72); 4
                            #(0.01 0.06 0.22 0.46); 5
                            ));;                  DEFENDING
...
(defun pick-change-branch (board move)
  (labels ((dice (pos)
             (cadr (aref board pos))))
    (let ((path (car move)))
      (if (or (null path)
...
(defun web-handle-human (pos)
  (cond ((not pos)
        ...
        (t
         (setf *cur-game-tree*
               (pick-change-branch (cadr *cur-game-tree*)
                                   (lazy-find-if (lambda (move)
                                                   (equal (car move)
                                                          (list *from-tile* pos)))
                                                 (caddr *cur-game-tree*))))
                                   ;;(lazy-nth pos (caddr *cur-game-tree*))))
...
(defun get-connected (board player pos)
  (labels ((check-pos (pos visited)
             (if (and (eq (car (aref board pos))
                 ...
                 ;; nie visited i jest to pole gracza!
                 (check-neighbors (neighbors pos) (cons pos
                                                        visited))
                 ;;return visited!
                 visited))
           (check-neighbors (lst visited)
             (if lst
                 ;; Dobra cos jeszcze zostalo,
                 ;;
                 (check-neighbors (cdr lst)
                                  (check-pos (car lst)
                                             visited))
                 ;;return visited
                 visited)))
