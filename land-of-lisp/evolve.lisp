...
;; test correcknes of this
;; on
;; 1 1 1 10 1 1 1 and see the probabilities
(defun turn (animal)
  (let ((x (random (apply #'+
                          (animal-genes animal)))))
...
(defun eat (animal)
  (let ((pos (cons (animal-x animal)
    ...
    (when (gethash pos *plants*)
      (incf (animal-energy animal) *plant-enery*)
      (remhash pos *plants*))))
...
(defun evolution ()
  ...
  (let ((str (read-line)))
    (cond ((equal str "quit")
           ())
          (t
           (let ((x (parse-integer str :junk-allowed t)))
             (if x
                 (loop for i
                    below x
                    do (update-world)
